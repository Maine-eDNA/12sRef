...
### Abbagadabba Functions
Add functions and bits from abbagadabba that we'd like to include here


### Other desired pre-built functions
If there are any other scripts that already exist that we'd like to use, paste those here (after logging an issue). Make sure to record their source and appropriate citation, especially if these are fully external to Maine eDNA

## Workflow Outline
The following steps outline is purely a guide suggestion of layout and not meant to be restrictive or the end-all-be-all of organization. Expand or adjust code and rearrange sections as needed, just log the changes in issues and comments and do your best to make sure any moves won't have negative downstream effects in terms of code order

## Step 0: Installing and Initializing Packages, Apps, and Other Required Tools
Put install/library checks of packages and anything else here (including non-data files needed to run a function) and please comment what the package or tool is doing.

## Step 1: Importing/Identifying Inputs
Add functions here to import csv lists (or other format) of species lists or other import-aligned functions

## Step 2: Cleaning Input
If we want any built in cleaning functions or name correcters, these should be next

## Step 3: Prepare GenBank Search
Creating the 12S search terms, voucher regular expression search, etc.

## Step 4: GenBank Searches

## Step 5: Filter Search Results
De-duplicate results and trim based on bp length

## Step 6: Create Taxonomy Dataframe

## Step 7: Create Sequence Files

## Step 8: Neighbor-Joining Phylogeny
